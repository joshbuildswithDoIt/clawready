...
## Problem
`clawreadynow.com` currently points to a **Vercel** IP (216.150.x.x) but was never deployed there.
...
2. Go to DNS settings for `clawreadynow.com`
3. Delete any existing A/CNAME records for `@` or `clawreadynow.com`
...
2. Click "Add custom domain" → enter `clawreadynow.com`
...
- Verify: `curl -I https://clawreadynow.com` should return 200

## Current Status
- `clawready.netlify.app` → ✅ Working
- `clawreadynow.com` → ❌ Vercel 404 (wrong DNS target)
